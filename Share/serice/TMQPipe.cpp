...
TMQPipe::TMQPipe(const char *pipe, TMQTopic *tmqTopic) : path{0}
{
    ...
    tmqExecutor = ThreadExecutor::CreateExecutor(this);
    tmqExecutor->Wakeup();
...
TMQPipe::~TMQPipe()
{
    ThreadExecutor::ReleaseExecutor(tmqExecutor);
    delete pipeReader;
}
...
void TMQPipe::DispatchMessage(const PMessage &message)
{
    ...
    else if (message.type == TYPE_LONG_END)
    {
        ...
        TMQList<PMessage *> senderMessages;
        ...
        for (int i = 0; i < senderMessages.Size(); ++i)
        {
            longMessages.Remove(senderMessages.Get(i));
...
void TMQPipe::OnRegister(unsigned char *info, unsigned int len)
{
    ...
    {
        if (strcmp(pipeReceivers.Get(index)->name, name) == 0 && strcmp(pipeReceivers.Get(index)->pipe, pipe) == 0)
        {
            id = index;
            break;
        }
